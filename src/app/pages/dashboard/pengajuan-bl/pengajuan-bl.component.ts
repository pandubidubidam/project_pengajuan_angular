...
@Component({
  selector: 'app-pengajuan-bl',
  templateUrl: './pengajuan-bl.component.html',
  styleUrls: ['./pengajuan-bl.component.css']
})
export class PengajuanBlComponent implements OnInit {
    ...
    if (fileInput && fileInput.files && fileInput.files.length > 0) {
      ...
      if (file.size > 5242880) { // Size in bytes (5MB)
        ...
        if (extension === 'pdf') {
          ...
          } else if (documentName === 'LYDS') {
            uploadDocument.docCode = '16';
          }
  ...
  submit(){
    ...
    } else if (this.services.uploadDocumentEditLYDS.docValue.length < 1) {
      Swal.fire({
          icon: 'error',
          title: 'Gagal',
          text: 'Dokumen Lampiran yang Disesuaikan Harus Di Upload!',
      })
    } else {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                console.log("this.services.uploadDocumentEditLYDS =====> ", this.services.uploadDocumentEditLYDS)
                console.log('submit')
                this.ajukan()
            }
        })
  ...
  ajukan(){
    this.services.ajukan.jenisPengajuan = 'BL';
    ...
    this.services.ajukan.document = [
      ...
      { docId: this.services.uploadDocumentEditLYDS.docId }
    ];
