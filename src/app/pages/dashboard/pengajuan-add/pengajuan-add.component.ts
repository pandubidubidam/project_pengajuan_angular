...
@Component({
  selector: 'app-pengajuan-add',
  templateUrl: './pengajuan-add.component.html',
  styleUrls: ['./pengajuan-add.component.css']
})
export class PengajuanAddComponent implements OnInit {
    ...
    if (fileInput && fileInput.files && fileInput.files.length > 0) {
      ...
      if (file.size > 5242880) { // Size in bytes (5MB)
        ...
        if (extension === 'pdf') {
          ...
          } else if (documentName === 'CKDDS') {
            uploadDocument.docCode = '9';
          } else if (documentName === 'SRB') {
            uploadDocument.docCode = '10';
          } else if (documentName === 'SPKDDDT') {
            uploadDocument.docCode = '12';
          } else if (documentName === 'SRDC') {
            uploadDocument.docCode = '13';
          }
          this.spinner.show();
  ...
  submit(){
    ...
    } else if (this.services.uploadDocumentEditCKDDS.docValue.length < 1) {
      Swal.fire({
          icon: 'error',
          title: 'Gagal',
          text: 'Dokumen Checklist Kelengkapan Dokumen dari SKPD Harus Di Upload!',
      })
    } else if (this.services.uploadDocumentEditSRB.docValue.length < 1) {
      Swal.fire({
          icon: 'error',
          title: 'Gagal',
          text: 'Dokumen Salinan Referensi Bank Harus Di Upload!',
      })
    } else if (this.services.uploadDocumentEditSPKDDDT.docValue.length < 1) {
      Swal.fire({
          icon: 'error',
          title: 'Gagal',
          text: 'Dokumen Surat Pernyataan Kelengkapan Dokumen dari Dinas Teknis Terkait Harus Di Upload!',
      })
    } else if (this.services.uploadDocumentEditSRDC.docValue.length < 1) {
      Swal.fire({
          icon: 'error',
          title: 'Gagal',
          text: 'Dokumen Surat Rekomendasi dari Camat yang Membawahi Desa Terkait Harus Di Upload!',
      })
    } else {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                console.log("this.services.uploadDocumentEditCKDDS =====> ", this.services.uploadDocumentEditCKDDS)
                console.log("this.services.uploadDocumentEditSRB =====> ", this.services.uploadDocumentEditSRB)
                console.log("this.services.uploadDocumentEditSPKDDDT =====> ", this.services.uploadDocumentEditSPKDDDT)
                console.log("this.services.uploadDocumentEditSRDC =====> ", this.services.uploadDocumentEditSRDC)
                console.log('submit')
                this.ajukan()
            }
        })
  ...
  ajukan(){
    this.services.ajukan.jenisPengajuan = 'ADD';
    ...
    this.services.ajukan.document = [
      ...
      { docId: this.services.uploadDocumentEditCKDDS.docId },
      { docId: this.services.uploadDocumentEditSRB.docId },
      { docId: this.services.uploadDocumentEditSPKDDDT.docId },
      { docId: this.services.uploadDocumentEditSRDC.docId }
    ];
