...
@Component({
  ...
  templateUrl: './pengajuan-gu.component.html',
  styleUrls: ['./pengajuan-gu.component.css']
})
export class PengajuanGuComponent implements OnInit {
  ...
  submit(){
    ...
    } else if (this.services.uploadDocumentEditLPJUP.docValue.length < 1) {
      Swal.fire({
          icon: 'error',
          title: 'Gagal',
          text: 'Dokumen Laporan Pertanggungjawaban (LPJ) UP Harus Di Upload!',
      })
    } else {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                console.log("this.services.uploadDocumentEditLPJUP =====> ", this.services.uploadDocumentEditLPJUP)
                console.log('submit')
                this.ajukan()
            }
        })
  ...
  ajukan(){
    this.services.ajukan.jenisPengajuan = 'GU';
    ...
    this.services.ajukan.document = [
      ...
      { docId: this.services.uploadDocumentEditLPJUP.docId }
    ];
